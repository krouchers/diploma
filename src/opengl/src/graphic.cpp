    ...
    : window_{win}, m_camera{cam}
{
    ...
    gl_context_ = SDL_GL_CreateContext(window_->get_handler());
    if (!gl_context_)
    {
    ...
    shader_.set("tex1", 0);
    shader_.set("tex2", 1);
    shader_.set("transform", mat4x4::get_rotation_matrix(vec3{0, 0, 30}) * mat4x4::get_projection_matrix(2.f, 10, 45, window_->get_aspect_ratio()));
}
